...
class TaskDAO {
  ...
  async createMainTask(taskData) {
    ...
    await this.createTaskLog(docRef.id, {
  ...
  async updateMainTask(taskId, updateData) {
    ...
    // log update
    await this.createTaskLog(taskId, {
  ...
  async deleteMainTask(taskId) {
    ...
    // delete main task
    batch.delete(db.collection("mainTasks").doc(taskId));
  ...
  async updateTaskProgress(taskId) {
    ...
    await db.collection("mainTasks").doc(taskId).update({
      progress,
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    });
  ...
  async addSubTaskLogTime(taskId, subTaskId, timeToAdd) {
    const subTaskRef = db
      .collection("mainTasks")
      .doc(taskId)
      .collection("subtasks")
    ...
    await subTaskRef.update({
      timeLogged: admin.firestore.FieldValue.increment(timeToAdd),
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    });

    // log time addition
    await this.createTaskLog(taskId, {
      action: "ADD_SUB_TASK_TIME",
      message: `Added ${timeToAdd} minutes to sub task`,
      subTaskId,
      timeAdded: timeToAdd,
    });
    return true;
